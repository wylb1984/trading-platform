...
2. 启动系统后，在页面里的“通知配置”中直接连接通道：
   - 微信：点击 `连接微信`，按提示扫码，再点击 `完成微信连接`
   - 飞书：点击 `连接飞书`，系统会自动识别当前已授权的飞书账号并绑定
3. 绑定成功后，只需要设置：
   - `是否启用自动通知`
   - `最低置信度`

环境变量仍然可以保留为默认值，但 GitHub 版本推荐通过页面完成绑定。若你想在首次启动前预设默认值，也可以在 `.env.local` 中配置：
